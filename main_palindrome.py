...
async def run(uids: list[str], proxy: str = None, use_pool: bool = False):
    total = len(uids)
    ...
    with open(f"uid_palindrome_{len(uids[0])}d.txt", "w", encoding="utf-8") as f:
    ...
    print(f"\n完成！共扫描 {total} 个回文 UID，找到 {len(found)} 个符合条件的账号")


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="扫描回文 UID")
    parser.add_argument("--pool", action="store_true", help="使用免费代理池")
    parser.add_argument("--proxy", type=str, help="手动指定代理 (http://host:port)")
    parser.add_argument("--digits", "-d", type=int, default=None, help="回文位数 (默认7)")
    parser.add_argument("--lo", type=int, default=None, help="UID 下限")
    parser.add_argument("--hi", type=int, default=None, help="UID 上限")
